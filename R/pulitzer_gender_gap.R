...
roll_sex <- data %>%
  ...
  filter(!is.na(winner)) %>%
  group_by(author_sex) %>%
  mutate(roll = cumsum(!is.na(author_sex)),
...
p <- roll_sex %>%
  ...
  labs(title = "**Pulitzer's Fiction**",
       ...
       caption = glue("The Fiction category was created in 1948; data from prior years ",
                      "shown here are from the Novel category. There were ten years during this span ",
                      "where no award was given. Analysis and graphic by Spencer Schien (@MrPecners), ",
                      "data from The Pulitzer Prizes and Wikipedia."))


ggsave(p, filename = "plots/pulitzer_sex_gap.png",
       w = 9, h = 5, bg = "white")
